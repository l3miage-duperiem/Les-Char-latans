...
Elles nous permettent d'extraire avec précision les chars recherchés par leurs intitulés, et ainsi générer différents graphiques qui sépare les valeurs possibles des données en classes (intervalle de valeurs prises par un caractère quantitatif) ou groupes.
...
-   **abandonned** : montre des signes d'abandon, souvent en combinaison avec des dégâts. L'équipement abandonné dans une zone sûre controlée par seulement l'un des deux camps ne figure pas dans ce jeu de données.
...
ggplot(rus_time_series, aes(x = date_recorded, y = cum_pertes, color = system_family)) +
  ...
  labs(
    title = "Cumulative losses of russian MBT per MBT serie since the beginning of the conflit",
    x = "Date",
    y = "Cumulative losses",
    color = "Serie"
  ) +
...
ggplot(ukr_time_series, aes(x = date_recorded, y = cum_pertes, color = system_family)) +
  ...
  scale_color_manual(values = famille_couleurs) +
  labs(
    title = "Cumulative losses of ukrainian MBT per MBT serie since the beginning of the conflit",
    x = "Date",
    y = "Cumulative losses",
    color = "Serie"
  ) +
...
totals_by_country_by_age <- all_MBTs %>%
  ...
  ggtitle("Number of MBT losses per country")
...
totals_by_country_by_age_proportion <- all_MBTs %>%
  ...
  xlab("Nation") +
  ylab("Percentage of all MBT losses") +
  ggtitle("Distribution of MBT generation per country in the losses")
...
ggplot(status_breakdown, aes(x = 1, y = pct, fill = status)) +
  ...
  facet_wrap(~ country) +
  labs(
    title = "Distribution of MBT losses status",
    fill = "Status"
  ) +
  theme_void() +
  theme(
...
totals_losses_by_month <- all_MBTs_losses_per_month %>%
  ...
  xlab("Time (in month)") +
  ylab("Number of visually confirmed losses") +
  ggtitle("Evolution of the number of MBT losses per month")
...
all_equipment_totals_losses_by_month <- all_vehicles %>%
  ...
  xlab("Time (in month)") +
  ylab("Number of visually confirmed losses") +
  ggtitle("Evolution of the number of all equipment losses per month")
...
4.  Évolution des proportions de générations de MBT et types de perte
...
Pour la recherche des faits historiques, l'usage de l'IA a été employé afin de corréler avec les tendances observées dans nos graphiques.
